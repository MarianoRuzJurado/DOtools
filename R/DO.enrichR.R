...
#' @param pval_cutoff adjusted p-value threshold for significance
#' (default = 0.05).
#' @param log2fc_cutoff log2 fold change threshold for up/down regulation
#' (default = 0.25).
#' @param path folder path where the output Excel file will be saved. A
#' subfolder "GSA_Tables" will be created.
#' @param filename suffix used in the Excel filename
#' (e.g., "GSA_CellType_MyAnalysis.xlsx").
#' @param species species name for enrichment analysis. Options include "Human",
#'  "Mouse", "Yeast", etc. (default = "Mouse").
#' @param go_catgs GO databases to use.
#' Defaults to c(GO_Biological_Process_2023").
#'
#' @return data.frame with GO enrichment results if `path` is NULL, otherwise
...
DO.enrichR <- function(df_DGE,
  ...
  df_up <- df_DGE[df_DGE[[pval_column]] < pval_cutoff &
    df_DGE[[log2fc_column]] > log2fc_cutoff, ]
  df_down <- df_DGE[df_DGE[[pval_column]] < pval_cutoff &
    df_DGE[[log2fc_column]] < -log2fc_cutoff, ]
  ...
  # Run enrichment
  res_up <- enrichR::enrichr(
    genes = df_up[[gene_column]],
    databases = go_catgs
  )
  res_down <- enrichR::enrichr(
    genes = df_down[[gene_column]],
    databases = go_catgs
  )
  ...
  if (!is.null(path)) {
    dir.create(file.path(path, "GO_Tables"),
      showWarnings = FALSE,
      recursive = TRUE
    )
