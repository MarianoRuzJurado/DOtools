...
DO.Import <- function(pathways,
    ...
    for (i in seq_along(pathways)) {
        ...
        mtx <- tryCatch(
            ...
            error = function(x) {
                ...
                tryCatch(
                    ...
                    error = function(x2) {
                        warning(
                            "Matrix is not produced by CellBender. ",
                            "Trying simple CSV read..."
                        )
                        tryCatch(
                            {
                                read.csv(
                            ...
                            error = function(e) {
                                stop(
                                    "Failed to read file as CellRanger, ",
                                    "CellBender, or CSV. Check file format."
                                )
                            }
                        )
                    }
        ...
        .logger(paste0(
            "Setting condition in object to: ",
            sub("[-|_].*", "", id)
        ))
        ...
        if (FilterCells == TRUE) {
            ...
            # check if absolute values are set for counts and quantile
            if (!is.null(min_counts) && !is.null(low_quantile)) {
                stop(
                    "Both filtering minimum counts by ",
                    "absolute values and quantile is set to Value!"
                )
            }

            if (!is.null(max_counts) && !is.null(high_quantile)) {
                stop(
                    "Both filtering maximum counts by absolute ",
                    "values and quantile is set to Value!"
                )
            }
        ...
        if (DeleteDoublets == TRUE) {
            ...
            SCE_obj <- .suppressAllWarnings(as.SingleCellExperiment(sce_object))
    ...
    if (Seurat == FALSE) {
        ...
        merged_obj[["RNA"]] <-
            split(merged_obj[["RNA"]], f = merged_obj$orig.ident)
    }
