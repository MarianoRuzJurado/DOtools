...
DO.Dotplot <- function(sce_object,
  ...
  if (is.vector(Feature) && annotation_x == TRUE) {
    stop(
      "Feature is a vector, but annotation_x is set to TRUE. If annotation ",
      "on xaxis is wanted with specific cluster names you need to provide a ",
      "dataframe with a column containing cluster names for the genes, ",
      "like in Seurat::FindAllMarkers!"
    )
  }
  ...
  if (!is.vector(Feature)) {
    ...
    if (is.null(cluster) || is.null(Feature)) {
      stop(
        "Couldn't derive Cluster and Feature information from the provided ",
        "Dataframe. \n Supported names for cluster: cluster|group|annotation|",
        "cell\n Supported names for Feature: gene|feature. \n Please make ",
        "sure that your colnames in the provided Dataframe are supported."
      )
    }
  }
  ...
  if (is.null(group.by.x) && !is.null(group.by.y) && is.null(group.by.y2)) {
    stop(
      "If you want to make a Marker Plot with just one provided group.by ",
      "then please use group.by.x!"
    )
  }
  ...
  if (identical(as.vector(data.plot.res$id), as.vector(data.plot.res$xaxis)) &&
    FeatureType == "list") {
    # go over input type
    ...
    data.plot.res$gene <-
      factor(data.plot.res$gene,
        levels = orig_DF[order(orig_DF$cluster, decreasing = FALSE), ]$gene
      )
      ...
      if (names_x == "Pseudobulk") {
        ...
        data.plot.res <- data.plot.res %>%
          ...
          mutate(
            p_adj = case_when(
              xaxis == names_x &
                !is.na(as.numeric(p_val_adj)) ~ as.numeric(p_val_adj),
  ...
  if (across.group.by.x == TRUE) {
    pmain <- pmain +
      ggplot2::geom_point(ggplot2::aes(fill = fill.values, size = pct.exp),
        shape = 21,
        stroke = point_stroke
      ) +
      guides.layer +
  ...
  if (!is.null(data.plot.res$stars)) {
    pmain <- pmain +
      ...
      scale_shape_manual(values = c("p < 0.05" = 8), name = "Significance") +
  ...
  if (annoManual == FALSE) {
    if (annoPos %in% c("top", "botomn")) {
      if (aesGroup == FALSE) {
        ...
        # safer indexing
        start <- c(1, groupInfo$Freq[seq_len(length(groupInfo$Freq) - 1)]) %>%
          cumsum()
