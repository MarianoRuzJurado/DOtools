#' @title DO.PyEnv
#' @description Sets up or connects to a conda Python environment for use with
...
#'
#' @param conda_path character string specifying the path to an existing or new
#' conda environment.
#' @return None
#'
#' @examples
...
DO.PyEnv <- function(conda_path = NULL) {
  ...
  if (!dir.exists(conda_path)) {
    ...
    conda_args1 <- c(
      ...
      "-y",
      "-p",
      file.path(path.expand("~"), ".venv/DOtools"),
      "python=3.11"
    )
    conda_args2 <- c(
      "conda",
