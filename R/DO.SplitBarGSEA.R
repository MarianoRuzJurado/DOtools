...
#' @description This function generates a split barplot. This is a plot where
...
#' @param col_split column in the dataframe that will be used to sort and split
#' the plot
#' @param cond_col column in the dataframe that contains the condition
#' information
...
#' @param log10_transform if col_split contains values between 0 and 1, assume
...
#' @param spacing space to add between bars and origin. It is a percentage value
...
#' @param colors_pairs colors for each condition
#' (1st color --> negative axis; 2nd color --> positive axis)
#' @param title title of the plot
#' @param showP if False, the axis is return
#' @param celltype vector with cell types you want to subset for, use "all" for
...
DO.SplitBarGSEA <- function(df_GSEA,
  ...
  if (!"celltype" %in% colnames(df_GSEA)) {
    stop(
      "Provided data frame has no column named 'celltype'! Please add a ",
      "column with cell type information."
    )
  }
  ...
  results <- basilisk::basiliskRun(env = DOtoolsEnv, fun = function(args) {
    ...
    for (celltype in unique(df_GSEA$celltype)) {
      ...
      df_GSEA_sub$Term <- vapply(
        strsplit(
          df_GSEA_sub[[term_col]], " \\(GO"
        ),
      ...
      if (!is.null(path)) {
        ...
        plt$savefig(paste0(path, celltype, "_", filename),
                    dpi = 300,
                    bbox_inches = "tight"
        )
        plt$close()
