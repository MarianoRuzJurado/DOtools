...
DO.VlnPlot <- function(sce_object,
  ...
  if (SeuV5 == TRUE) {
    rlang::warn(
      "SeuV5 is TRUE, if working with Seuratv4 or below change SeuV5 to FALSE",
      .frequency = "once",
      .frequency_id = "v5Mean"
    )
    if (Feature %in% rownames(sce_object)) {
  ...
  if (is.null(group.by.2)) {
    ...
    if (nrow(group_of_zero) > 0) {
      ...
      df.melt <- df.melt %>%
        ...
        dplyr::mutate(
          value = if_else(
            row_number() == 1 & all(value == 0), .Machine$double.xmin, value
          )
        ) %>%
        ungroup()
    }
            ...
            y.position = seq(
              from = max(
                sce_object@assays$RNA$data[Feature, ][
                  !is.na(sce_object@assays$RNA$data[Feature, ])
                ]
              ) * stat_pos_mod,
              by = step_mod,
